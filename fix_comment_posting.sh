...
# Fix comment posting errors

echo "🔧 FIXING COMMENT POSTING..."
echo "============================="
...
class _CommentsScreenState extends State<CommentsScreen> {
    ...
    try {
      ...
      if (doc.exists) {
        ...
        if (commentsData is List) {
          ...
          _comments.sort((a, b) {
            final timeA = a['timestamp'] is Timestamp ? (a['timestamp'] as Timestamp).toDate() : DateTime.now();
            final timeB = b['timestamp'] is Timestamp ? (b['timestamp'] as Timestamp).toDate() : DateTime.now();
  ...
  Widget _buildCommentItem(Map<String, dynamic> comment) {
    ...
    } else if (comment['timestamp'] is Map) {
      // Handle Firestore timestamp map
      final tsMap = comment['timestamp'] as Map<String, dynamic>;
      final seconds = tsMap['_seconds'] as int? ?? 0;
      final nanoseconds = tsMap['_nanoseconds'] as int? ?? 0;
      timestamp = DateTime.fromMillisecondsSinceEpoch(seconds * 1000 + nanoseconds ~/ 1000000);
    } else {
...
echo ""
echo "✅ COMMENT POSTING FIXED"
echo "========================"
echo "Improved error handling and Firestore data structure handling"